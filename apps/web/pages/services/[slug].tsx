...
export function Slug({ preview, slug }) {
  const { data } = useQuery([PAGE_TYPES.services, slug], () => new FetchServiceBySlug(slug).fetch(preview));
...
export const getStaticProps: GetStaticProps = async ({ locale, params, preview = false }) => {
  ...
  await queryClient.prefetchQuery([PAGE_TYPES.services, slug], () => new FetchServiceBySlug(slug).fetch(preview));
