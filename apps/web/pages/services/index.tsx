...
export function Index({ preview }) {
  const { data } = useQuery([PAGE_TYPES.services], () => new FetchPageBySlug(PAGE_TYPES.services, '').fetch(preview));
  usePagePreview({ pageType: PAGE_TYPES.services, slug: '', preview })
...
export const getStaticProps: GetStaticProps = async ({ locale, preview = false }) => {
  ...
  await queryClient.prefetchQuery([PAGE_TYPES.services], () => new FetchPageBySlug(PAGE_TYPES.services, '').fetch(preview));
