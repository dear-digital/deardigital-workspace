...
import { FetchPodcastBySlug, fetchDatasourceBySlug, fetchPagePaths } from '@deardigital/shared/services';
...
export function Slug({ preview, slug }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetchDatasourceBySlug('i18n', "en", true);
      console.log(res);
    }
    fetchData()
  }, [])
...
export const getStaticProps: GetStaticProps = async ({ locale, params, preview = false }) => {
  ...
  await queryClient.prefetchQuery([PAGE_TYPES.podcast, slug], () => new FetchPodcastBySlug(slug).fetch(preview));
