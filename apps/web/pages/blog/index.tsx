...
export function Index({ preview }) {
  const { data } = useQuery([PAGE_TYPES.blog], () => new FetchPageBySlug(PAGE_TYPES.blog, '').fetch(preview));
...
export const getStaticProps: GetStaticProps = async ({ locale, preview = false }) => {
  ...
  await queryClient.prefetchQuery([PAGE_TYPES.blog], () => new FetchPageBySlug(PAGE_TYPES.blog, '').fetch(preview));
