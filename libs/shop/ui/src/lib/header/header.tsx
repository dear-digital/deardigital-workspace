...
import { NavigationLink } from '@deardigital/shared/ui';
    ...
    <nav className="bg-white border-gray-200 dark:bg-gray-900">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link href="/" className="flex items-center">
          <img src="/brand-logo.svg" className="h-8 mr-3" alt="Flowbite Logo" />
          <span className="sr-only">Knoll</span>
        </Link>
        ...
        <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
          ...
          {globals?.header && (
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              {globals.header.navigation.map((item, key) => <NavigationLink key={key} {...item} />)}
            </ul>
          )}
        </div>
      </div>
